        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-category" class="form-horizontal">
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane fade" id="tab-data">
              <div class="form-group">
                  <label class="col-sm-2 control-label" for="input-related">category</label>
                  <div class="col-sm-10">
                      <input type="text" name="categorys" value="" placeholder="Category" class="form-control" />
                      <div id="category-related" class="well well-sm" style="height: 150px; overflow: auto;">
                          <?php if(!empty($category_id)) foreach ($category_id as $category) { ?>
                                  <div id="category<?php echo $category['category_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $category['title']; ?>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    ...
                    <div id="relatiedSolution" class="well well-sm" style="height: 150px; overflow: auto;">
                        <?php if(!empty($related_product_id)) foreach ($related_product_id as $product) { ?>
  ...
  <script type="text/javascript"><!--
      ...
      $('input[name=\'related\']').autocomplete({
          ...
          'select': function(item) {
              ...
              $('#relatiedSolution').append('<div id="relatiedSolution' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="related_product_id[]" value="' + item['value'] + '" /></div>');
          }
      });

      // Related
      $('input[name=\'categorys\']').autocomplete({
          'source': function(request, response) {
              $.ajax({
                  url: 'index.php?route=solution/solution/getCategory&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
                  dataType: 'json',
                  success: function(json) {
                      response($.map(json, function(item) {
                          return {
          ...
          'select': function(item) {
              $('input[name=\'categorys\']').val('');

              $('#category' + item['value']).remove();

              $('#category-related').append('<div id="category' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="category_id[]" value="' + item['value'] + '" /></div>');
          }
      });
//--></script>
  <script type="text/javascript"><!--
      $('#relatiedSolution,#category-related').delegate('.fa-minus-circle', 'click', function() {
          $(this).parent().remove();
      });
//--></script>
  <script type="text/javascript"><!--
