...
class ControllerModuleHomeQuote extends Controller
{
    public function index()
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/home_quote.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/module/home_quote.tpl', $data);
        } else {
            return $this->load->view('default/template/module/home_quote.tpl', $data);
        }
    }
    ...
    public function sendQuote(){
        ...
        $verCode = $_SESSION['getHomeQuote'];
        if($post['homeVer'] != $verCode){
            $this->response->addHeader('Content-Type: application/json');
            $json['status'] = 'error';
            $json['info'] = 'verification code error';
            $this->response->setOutput(json_encode($json));
        }else{
            $tocustom = $this->load->view($this->config->get('config_template') . '/mail/home_config_quote_to_coustom.html', $post);
            $post['ip'] = getIP();
            $tous = $this->load->view($this->config->get('config_template') . '/mail/home_config_quote_to_us.html',$post);
            $json['info'] = $this->sendmail($this->config->get('config_mail_parameter'),"The new enquiries",$tous);
            $json['info'] = $this->sendmail($this->request->post['Email'],"Your Enquiry has been recorded",$tocustom);
            $this->response->addHeader('Content-Type: application/json');
